...
function withDemoHosts(realProfiles: Profile[]) {
  ...
  // Demo/preview identities are development-only. Production discovery must contain only verified Hosts.
...
export function DiscoverBrowser() {
  ...
  const renderCard = (profile: Profile, swipe = false) => (
    <article className="discover-card" key={profile.host_id} style={{ width: "100%", maxWidth: swipe ? 520 : 360, margin: swipe ? "0 auto" : 0, overflow: "hidden", borderRadius: 24, background: "var(--surface)", border: "1px solid var(--line)" }}>
      <div className="discover-photo" role="img" aria-label={`${profile.display_name} profile photo`} style={{ aspectRatio: "4 / 5", width: "100%", maxHeight: swipe ? 620 : 450, overflow: "hidden", position: "relative", background: "var(--blush)" }}>
        {profile.avatar_path ? (
          <img src={profile.avatar_path} alt={`${profile.display_name} profile`} style={{ display: "block", width: "100%", height: "100%", objectFit: "cover", objectPosition: "center" }} />
        ) : (
          <div className="photo-placeholder" style={{ height: "100%", minHeight: 0 }}>Profile photo</div>
        )}
      </div>
      <div className="discover-info" style={{ padding: "1.15rem 1.2rem 1.25rem" }}>
        <div style={{ alignItems: "flex-start", display: "flex", justifyContent: "space-between", gap: "1rem" }}>
          <div>
        ...
        <p style={{ color: "var(--muted)", lineHeight: 1.55, margin: ".75rem 0 1rem", minHeight: swipe ? "auto" : "2.5rem" }}>{profile.headline || profile.bio || "A great conversation awaits."}</p>
        <div className="discover-actions" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: ".65rem" }}>
  ...
  return (
    <>
      <div className="form-footer" style={{ display: "flex", justifyContent: "center", gap: ".6rem", marginBottom: "1.4rem" }}>
        <Button type="button" variant={view === "cards" ? "primary" : "quiet"} onClick={() => setView("cards")}>Swipe</Button>
        <Button type="button" variant={view === "grid" ? "primary" : "quiet"} onClick={() => setView("grid")}>All profiles</Button>
      </div>
      {message && <p role="status" className="muted" style={{ textAlign: "center", marginBottom: "1rem" }}>{message}</p>}
      {view === "cards" ? (
        <section aria-label="Swipe discovery" style={{ maxWidth: 620, margin: "0 auto" }}>
          <div className="muted" style={{ marginBottom: ".7rem", textAlign: "center" }}>{profiles.length} available</div>
          {renderCard(current, true)}
        </section>
      ) : (
        <div className="discover-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 360px))", justifyContent: "center", gap: "1.2rem" }}>
