...
export function Modal({ title, children, closeLabel = "Close" }: { title: string; children: ReactNode; closeLabel?: string }) { return <div className="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="modal-title"><div className="modal"><div className="modal-heading"><h2 id="modal-title">{title}</h2><button type="button" aria-label={closeLabel}>×</button></div>{children}</div></div>; }
