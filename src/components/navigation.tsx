...
export function Brand({ light = false }: { light?: boolean }) { return <Link className="brand" href="/"><span className="brand-mark">I</span><span style={light ? { color: "var(--cream)" } : undefined}>PYAAR</span></Link>; }

export function BackControl({ href = "/", label = "Back to Pyaar" }: { href?: string; label?: string }) {
