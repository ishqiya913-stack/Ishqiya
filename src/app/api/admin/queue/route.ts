...
export async function GET(request: Request) {
  ...
  else if (kind === "hosts") query = admin.from("profiles").select("id,email,display_name,role,account_status,created_at,host_profiles!left(is_active,is_visible,is_discoverable,approval_status)", { count: "exact" }).eq("role", "host").order("created_at", { ascending: false });
