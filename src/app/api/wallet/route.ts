...
export async function GET() {
  ...
  const [{ data: wallet, error: walletError }, { data: packages, error: packageError }, { data: pendingOrder, error: orderError }] = await Promise.all([
    ...
    supabase.from("coin_packages").select("id, coins, price_paise").eq("is_active", true).order("coins"),
...
export async function POST(request: Request) {
  ...
  const { data: packageRow, error: packageError } = await admin.from("coin_packages").select("id, coins, price_paise").eq("id", body.packageId).eq("is_active", true).maybeSingle();
  ...
  const upiUri = `upi://pay?pa=${encodeURIComponent(upiId)}&pn=Ishqiya&am=${(packageRow.price_paise / 100).toFixed(2)}&cu=INR&tn=${encodeURIComponent(`Ishqiya ${orderId}`)}`;
  const { data: order, error } = await admin.from("payment_orders").insert({ id: orderId, user_id: user.id, package_id: packageRow.id, coins: packageRow.coins, expected_amount_paise: packageRow.price_paise, upi_uri: upiUri }).select("id, coins, expected_amount_paise, upi_uri, status, expires_at").single();
