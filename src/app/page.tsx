...
export default async function Home() {
  ...
  return <main className="landing-page"><div className="landing-shell"><header className="landing-header"><Link href="/" className="landing-brand" aria-label="IshqMe home"><span className="landing-brand-mark" aria-hidden="true">♥</span><span><span className="landing-wordmark">ISHQME</span><span className="landing-tagline">{content.brand_tagline}</span></span></Link><Link href="/auth/host/sign-up" className="landing-sign-in">Join as Host</Link></header>
    <section className="landing-hero"><div className="landing-hero-copy"><p className="landing-eyebrow">{content.hero_eyebrow}</p><h1>{content.hero_title}</h1><p className="landing-intro">{content.hero_intro}</p><div className="landing-points" aria-label="IshqMe highlights"><p><span aria-hidden="true">♥</span> {content.hero_feature_1}</p><p><span aria-hidden="true">✦</span> {content.hero_feature_2}</p></div><div className="landing-actions"><FastLoginButton /></div></div><div className="landing-hero-art"><div className="landing-image-frame"><Image src="/images/landing-model.png" alt="IshqMe connections" fill priority sizes="(max-width: 700px) 88vw, (max-width: 1100px) 43vw, 500px" className="object-contain object-center" /></div><p className="landing-image-note"><span>ISHQME</span> / connection, with intention</p></div></section>
    ...
    <footer className="landing-footer"><span>ISHQME</span><span>{content.brand_tagline}</span><span>Made for meaningful conversations</span></footer>
  </div></main>;
}
