...
export default function AdminSignIn() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        setError(result.error || `Admin sign-in failed (${response.status}).`);
        return;
      }
        ...
        <div className="rounded-3xl border border-white/10 bg-white/[0.045] p-7 shadow-2xl shadow-black/40 backdrop-blur-xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <input id="email" name="email" type="email" autoComplete="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="w-full rounded-xl border border-white/10 bg-black/30 px-4 py-3.5 text-sm text-white outline-none transition placeholder:text-zinc-600 focus:border-pink-500/60 focus:ring-2 focus:ring-pink-500/10" placeholder="admin@ishqiya.com" />
            </div>
            <div>
              <label htmlFor="password" className="mb-2 block text-xs font-semibold text-zinc-300">Password</label>
              <input id="password" name="password" type="password" autoComplete="current-password" required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full rounded-xl border border-white/10 bg-black/30 px-4 py-3.5 text-sm text-white outline-none transition placeholder:text-zinc-600 focus:border-pink-500/60 focus:ring-2 focus:ring-pink-500/10" placeholder="Enter your password" />
            </div>
            <div className="flex justify-end">
              <a href="/auth/admin/forgot-password" className="text-xs font-semibold text-pink-400 transition hover:text-pink-300">Forgot password?</a>
            </div>
            <button type="submit" disabled={loading} className="w-full rounded-xl bg-gradient-to-r from-pink-600 to-fuchsia-600 px-4 py-3.5 text-sm font-bold text-white shadow-lg shadow-pink-950/30 transition hover:from-pink-500 hover:to-fuchsia-500 disabled:cursor-not-allowed disabled:opacity-50">
              {loading ? "Signing in securely…" : "Sign in to Admin Console"}
            </button>
            {error && <div className="rounded-xl border border-red-500/20 bg-red-500/10 px-4 py-3 text-sm text-red-300" role="alert">{error}</div>}
          </form>
        </div>
