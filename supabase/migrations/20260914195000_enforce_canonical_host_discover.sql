-- Discover must use the same canonical Host state controlled by Admin.
-- admin_profiles may provide display content, but it must never bypass host_profiles controls.
